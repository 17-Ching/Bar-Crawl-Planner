<template>
  <div class="min-h-dvh max-w-[1280px] mx-auto">
    ...
    <section v-if="weeklyChamp" class="mx-4 mb-4">
      <div class="card p-4 border-neon-amber/30 bg-gradient-to-r from-neon-amber/5 to-transparent">
        <div class="flex items-center gap-2 mb-2">
          <span class="badge-amber">🏆 本週醉人攝影師</span>
        </div>
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-full overflow-hidden bg-dark-700 border border-neon-amber/40 shrink-0">
            <img v-if="weeklyChamp.user?.avatar_url" :src="weeklyChamp.user.avatar_url" class="w-full h-full object-cover" />
            <div v-else class="w-full h-full flex items-center justify-center">📸</div>
          </div>
          <div>
            <p class="font-semibold text-neon-amber text-sm">{{ weeklyChamp.user?.username }}</p>
            <p class="text-xs text-text-muted">{{ weeklyChamp.cheers ?? 0 }} 個舉杯 🍺</p>
          </div>
        </div>
      </div>
    </section>
    ...
    <section class="px-4 pb-4 mx-auto">
        ...
        >
          ...
          <div class="p-2.5">
            ...
            <div v-if="!photo.guessed" class="flex items-center gap-2 mb-2">
              ...
              />
              <button @click.stop="submitGuess(photo)" class="btn btn-primary btn-sm !px-2">猜</button>
            </div>
            ...
            <div class="flex items-center justify-between">
              <button
                ...
                :class="photo.cheered ? 'text-neon-amber' : 'text-text-muted hover:text-neon-amber'"
              >
      ...
      <div v-if="!loading && !photos.length" class="text-center py-16 text-text-muted">
...
async function load() {
  ...
  const { data } = await supabase
    .from('visit_history')
    .select('id, photo_urls, note, bar:bars(name), user:users(username, avatar_url)')
  ...
  // 週榜冠軍（取第一個有頭像的用戶作為 Demo）
  if (photos.value.length) weeklyChamp.value = photos.value[0]
  loading.value = false
}
...
function submitGuess(photo) {
  ...
  const isCorrect = photo.bar?.name?.toLowerCase().includes(photo.guess.toLowerCase())
  photo.guessed = true
  photo.guessResult = isCorrect
  if (isCorrect) success('猜對了！獲得 +5 XP 🎉')
}

function openPhoto(photo) { /* 可擴充全螢幕預覽 */ }
