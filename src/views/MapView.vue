<template>
  <div class="relative w-full flex flex-col h-[calc(100vh-80px)]">
    ...
    <header
      class="absolute top-0 inset-x-0 z-[900] p-3 pointer-events-none max-w-[1280px] mx-auto"
    >
      <div class="glass-card p-2.5 flex items-center gap-2 pointer-events-auto">
        <!-- App 標誌 -->
        <span class="font-display font-bold text-sm text-neon-gradient shrink-0"
          >🍺</span
        >
        ...
        <div class="flex-1 relative">
          <Search
            :size="14"
            class="absolute left-2.5 top-1/2 -translate-y-1/2 text-text-muted pointer-events-none z-10"
          />
          <input
            id="map-area-search"
            v-model="areaQuery"
      ...
      <div class="flex gap-2 mt-2 pointer-events-auto">
        ...
        >
          <Beer :size="15" />
          酒吧模式
          <span v-if="barCount > 0" class="text-xs opacity-70"
            >({{ barCount }})</span
          >
        </button>
        <button
        ...
        >
          ...
          超商模式
          <span v-if="storeCount > 0" class="text-xs opacity-70"
            >({{ storeCount }})</span
          >
        </button>
      </div>
    ...
    <Transition name="fade-overlay">
      ...
      >
        ...
        <h2
          class="font-display font-bold text-2xl text-neon-gradient mb-3 tracking-widest"
        >
          不醉不歸
        </h2>
        <div class="px-5 py-3 flex items-center gap-3">
          <div
            class="w-5 h-5 rounded-full border-2 border-neon-purple/30 border-t-neon-purple animate-spin"
          />
    ...
    <!-- ── 右下浮動按鈕 ── -->
    <div
      class="absolute bottom-[120px] left-1/2 -translate-x-1/2 z-[400] flex flex-col gap-2"
    >
      <button
        id="map-refresh-btn"
        @click="loadPOIs"
        class="w-11 h-11 rounded-xl glass-card border flex items-center justify-center text-text-muted hover:text-neon-cyan hover:border-neon-cyan/40 transition-all duration-200"
        aria-label="重新載入地標"
      >
    ...
    <Transition name="fade-overlay">
      <div
        v-if="barsStore.bars.length"
        class="absolute bottom-[60px] inset-x-3 z-[400] max-w-[1280px] mx-auto flex flex-col items-center"
      >
        <div
          class="glass-card px-4 py-2 flex items-center justify-between max-w-[500px] w-full"
        >
...
const DEMO_BARS = [
  {
    ...
    lng: 121.5632,
    category: "bar",
    tags: { opening_hours: "20:00–02:00" },
  },
  {
    id: "demo_2",
    ...
    lng: 121.5544,
    category: "bar",
    tags: { opening_hours: "19:00–02:00" },
  },
  {
    id: "demo_3",
    name: "Revolver",
    ...
    lng: 121.5607,
    category: "pub",
    tags: { opening_hours: "17:00–03:00" },
  },
  {
    id: "demo_4",
    name: "Draft Land",
    ...
    lng: 121.5578,
    category: "bar",
    tags: { opening_hours: "15:00–00:00" },
  },
  {
    id: "demo_5",
    name: "全家 FamilyMart",
  ...
  },
  {
    ...
    lng: 121.5513,
    category: "bar",
    tags: { opening_hours: "20:00–03:00" },
  },
  {
    id: "demo_7",
    name: "Indulge Bistro",
    ...
    lng: 121.562,
    category: "bar",
    tags: { opening_hours: "18:00–02:00" },
  },
  {
    id: "demo_8",
    name: "7-ELEVEN",
  ...
  },
  {
    ...
    lng: 121.5532,
    category: "pub",
    tags: { opening_hours: "17:00–02:00" },
  },
  {
    id: "demo_10",
    name: "Marquee Taipei",
    ...
    lng: 121.556,
    category: "bar",
    tags: { opening_hours: "22:00–06:00" },
  },
];
...
function debouncedLoadPOIs() {
  ...
  poiSearchTimeout = setTimeout(() => {
    if (mapLoading.value) return; // 若已有請求進行中，不重複觸發
    loadPOIs();
  }, 1500);
}
...
async function loadPOIs() {
  ...
  try {
    ...
    const all = await barsStore.fetchFromOverpass({
      lat: center.lat,
      lng: center.lng,
      radiusM,
      type: "all",
...
async function fetchSuggestions(q) {
  try {
    ...
    const res = await fetch(url, {
      headers: {
        "User-Agent": "BarCrawlPlannerApp/1.0",
